#> multipart-item:impl/set_data/sections/offhand
#
#   > Load the offhand section of the player's inventory, and modify the item stack that has the `{multipart-item: {add_part: 1b}}` NBT from the section
#
#@within tag/function *:impl/get_sections


#   Load the offhand section
function phi.modifyinv:load/offhand


#   Modify the item that has the `{multipart-item: {add_part: 1b}}` NBT
...
#   Apply the changes made to the player's offhand inventory section
data modify block -30000000 0 1602 Items set from storage multipart-item:global temp.container

execute if score #stacksPrepped m-i.main matches 1 run function phi.modifyinv:apply/offhand
