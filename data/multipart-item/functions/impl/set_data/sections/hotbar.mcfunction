#> multipart-item:impl/set_data/sections/hotbar
#
#   > Load the hotbar section of the player's inventory, and modify the item stacks that has the `{multipart-item: {add_part: 1b}}` NBT from the section
#
...
#   Apply the changes made to the player's hotbar inventory section
data modify block -30000000 0 1602 Items set from storage multipart-item:global temp.container

execute if score #stacksPrepped m-i.main matches 1 run function phi.modifyinv:apply/hotbar
